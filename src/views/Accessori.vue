<template>
  <span>
    <v-container grid-list-lg >
            <v-layout row wrap v-if=allMisc>
                <v-flex xs12 sm12 md6 lg4 xl3 v-for="misc in allMisc" :key="misc.id">
                            <v-hover>
                                <v-card
                                slot-scope="{ hover }"
                                ...
                                :to="'/details/' + misc.id"
                                >
                                <v-img :aspect-ratio="3/5" :src="misc.imglink" >
                                    <v-container fill-height fluid>
                                        <v-layout fill-height>
                                            <v-flex xs12 align-end flexbox>
                                                <v-avatar>
                                                    <img :src="getBrandIcon(misc.brand)">
                                                </v-avatar>
                                            </v-flex>
                                        </v-layout>
                                    </v-container>
                                ...
                                <v-card-title>
                                    <div>
                                        <span class="headline">{{ misc.name }}</span>
                                        <div class="d-flex">
                                            <!--<v-rating
                                            :value="misc.stars"
                                            color="amber"
                                            ...
                                            size="12"
                                            ></v-rating>-->
                                            <span class="justify-start">Reviews: {{ misc.nrevs }}</span>
...
export default {
  ...
  methods: {
      ...
      coll
        ...
        .then(snapshot => {
          snapshot.forEach(doc => {
            this.allMisc.push(doc.data()); // Messy with array positions and firebase IDs. // Solved with routing.
    ...
    getAvgStars(id) { // WIP
      //let arr = [];
      //arr = this.allMisc[id];
      this.allMisc[id].stars = this.allMisc[id].stars[0];
    },
    getBrandIcon(brand) {
      let src;
      switch (brand) {
